...
func createAndExecuteRequest(method, url string, body io.Reader, gladiaKey string) (*http.Response, error) {
...
func pollForTranscriptionResult(resultURL, gladiaKey string) (*TranscriptionResult, error) {
	spinner := []string{"-", "\\", "|", "/"}
	spinnerIndex := 0

	for {
		resp, err := createAndExecuteRequest("GET", resultURL, nil, gladiaKey)
		if err != nil {
			return nil, err
		}
...
func GetTranscription(transcriptionRequest TranscriptionRequest) (*TranscriptionResult, error) {
	...
	resp, err := createAndExecuteRequest("POST", "https://api.gladia.io/v2/transcription", bytes.NewBuffer(requestBody), gladiaKey)
	if err != nil {
		return nil, err
	}
	defer resp.Body.Close()
	...
	return pollForTranscriptionResult(transcriptionResponse.ResultURL, gladiaKey)
}
